...
- **Auto-Tagging**: Automatically adds relevant tags to posts published without tags based on content matching
...
4. **Auto-Tagging Feature**:
   - When you publish a post without any tags, the plugin automatically analyzes the content
   - It searches through all existing tags and matches them against the post content
   - **Smart Fuzzy Matching**: Handles Greek grammatical variations (e.g., "Μέση Ανατολή" matches "Μέσης Ανατολής")
   - **Multi-language Support**: Works with Greek, English, and other UTF-8 languages
   - The top 10 most relevant tags (based on word frequency in the content) are automatically added
   - This feature only works when publishing posts with NO tags - it won't override manual tag selection
   - Weighs title matches higher than content matches for better relevance
...
│   └── class-auto-tagger.php    # Auto-tagging functionality
├── templates/
...
- `publish_post`: Auto-tag posts when published without tags
- `save_post`: Check and auto-tag posts on save
...
- **NEW**: Auto-tagging feature - Automatically adds relevant tags when publishing posts without tags
- **NEW**: Fuzzy matching support for Greek and international languages
- **NEW**: Handles grammatical variations (e.g., Μέση Ανατολή matches Μέσης Ανατολής)
- Improved UTF-8 character support
